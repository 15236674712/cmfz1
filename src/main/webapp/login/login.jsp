    ...
    <script>
        $(function () {
            ...
            $("#captchaImage").click(function () {
                $("#captchaImage").prop("src", "${path}/user/getImageCode?time=" + new Date().getTime());
            });

            /*表单验证*/
            $.extend($.validator.messages, {
                //表单验证空展示错误信息     将要验证的input框加入  required
                required: "<span style='color: red'>&nbsp*&nbsp必填字段</span>",
                //表单验证长度展示错误信息   将要验证的input框加入  minlength="2"
                minlength: $.validator.format("<span style='color: red'>&nbsp至少2字符</span>"),
            });
                ...
                if (isOk) {
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.success != 200) {
                                $("#msgDiv").html("<span id='checkId' class='error' style='color:red'>" + data.message + "</span>");
                            } else {
                    ...
                    <div class="form-bottom" style="width: 450px">
                        <form role="form" action="" method="post" class="login-form" id="loginForm">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" minlength="2" name="username" placeholder="请输入用户名..."
                                       class="form-username form-control" required id="form-username">
                            </div>
                            <div class="form-group">
                                <label class="sr-only" for="form-password">Password</label>
                                <input type="password" minlength="2" name="password" placeholder="请输入密码..."
                                       class="form-password form-control" required id="form-password">
                            </div>
                            <div class="form-group">
                                <%--<label class="sr-only" for="form-code">Code</label>--%>
                                <img id="captchaImage" style="height: 48px;width:100px" class="captchaImage"
                                     src="${path}/user/getImageCode">
                                <input style="width: 289px;height: 50px;border:3px solid #ddd;border-radius: 4px;"
                                       required minlength="2" type="test" name="enCode" id="form-code">
                            </div>
